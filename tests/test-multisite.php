...
class WP_DownloadManager_Multisite_Test extends WP_DownloadManager_TestCase {
	...
	private function seed_network( $count = 2 ) {
		...
		foreach ( $site_ids as $blog_id ) {
			...
			$wpdb->query( "DROP TABLE IF EXISTS {$wpdb->downloads}" );
	...
	public function test_network_activation_installs_on_every_site() {
		...
		foreach ( $site_ids as $blog_id ) {
			switch_to_blog( $blog_id );
			// Both sides have to be read inside the switch: $wpdb->downloads
			// is re-prefixed on the way back out.
			$expected = $wpdb->downloads;
			$exists   = $wpdb->get_var( $wpdb->prepare( 'SHOW TABLES LIKE %s', $expected ) );
			restore_current_blog();

			$this->assertSame( $expected, $exists, "Site {$blog_id} did not get its downloads table." );
		}
	}
